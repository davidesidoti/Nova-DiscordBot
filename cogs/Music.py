...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def get_youtube_playlist_videos(cls, playlist_id):
...
class Music(commands.Cog):
    ...
    async def play_(self, ctx, *, search: str):
        ...
        if 'youtube.com/playlist' in search:
            playlist_id = re.findall(r'list=([a-zA-Z0-9_-]+)', search)[0]
            videos = await YTDLSource.get_youtube_playlist_videos(playlist_id)
            for video in videos:
                src = {'webpage_url': video["url"], 'requester': ctx.author, 'title': video["title"]}
    ...
    async def queue_info(self, ctx):
        ...
        upcoming = list(itertools.islice(player.queue._queue,
                        0, 20)) #! int(len(player.queue._queue)) al posto del venti
        fmt = '\n'.join(
